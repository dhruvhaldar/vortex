...
const securityHeaders = [
  ...
  },
  {
    ...
    value: `default-src 'self'; script-src 'self' 'unsafe-inline' 'wasm-unsafe-eval' ${process.env.NODE_ENV === 'development' ? "'unsafe-eval'" : ""}; script-src-attr 'none'; connect-src 'self' data: blob:; style-src 'self' 'unsafe-inline'; img-src 'self' blob: data:; object-src 'none'; base-uri 'none'; form-action 'none'; frame-ancestors 'none'; worker-src 'none'; media-src 'none'; child-src 'none'; frame-src 'none'; manifest-src 'none'; prefetch-src 'self'; upgrade-insecure-requests;`.replace(/\s+/g, ' ').trim(),
  },
  {
