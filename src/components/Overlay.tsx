  ...
  useFrame(() => {
    if (progressRef.current && scroll) {
      ...
      if (scroll.offset !== lastOffset.current) {
        ...
        // ⚡ Bolt: Throttle aria-valuenow updates to integer percentages to avoid overwhelming the accessibility tree
        const percent = Math.round(scroll.offset * 100);
        if (percent !== lastPercent.current) {
          progressRef.current.setAttribute('aria-valuenow', percent.toString());
