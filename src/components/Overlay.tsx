    ...
    if (progressRef.current && scroll) {
      ...
      if (scroll.offset !== lastOffset.current) {
        ...
        if (progressContainerRef.current) {
          ...
          // Only update aria-valuenow if the integer percentage changes to avoid overwhelming screen readers
          if (percentage !== lastAriaValue.current) {
            progressContainerRef.current.setAttribute('aria-valuenow', percentage.toString());
            lastAriaValue.current = percentage;
          }
        }
      }
    }
