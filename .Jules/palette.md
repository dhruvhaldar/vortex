...
## 2025-10-27 - Semantic ARIA for Canvas Rendered Scroll Progress Bars
**Learning:** Custom scroll progress bars tied to `useFrame` render loops lack semantic meaning for screen readers and can overwhelm assistive technologies if ARIA attributes are updated too frequently.
**Action:** When implementing custom visual scroll progress bars in a continuous render loop, add `role="progressbar"` and related `aria-*` attributes (`aria-valuenow`, `aria-valuemin`, `aria-valuemax`). To prevent overwhelming screen readers with high-frequency updates, update `aria-valuenow` via a ref and throttle the updates (e.g., only updating when the integer percentage changes).
